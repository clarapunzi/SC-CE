...
for meth in ["random","kdtree"]:
    ...
    for j,i in enumerate(percs):
        ...
        percentages = exp["random_forest"][meth][0].sum()/total_samples*100
        print(percentages)
    ...
    plt.suptitle("model random_forest\n Ablation for dice_ml \""+
                 str(meth)+
                 "\" \nfor "+str(total_samples)+" samples",fontsize=18)
    plt.tight_layout()
    # show the grid
    plt.grid(True)
    ...
    # save all the relevant data in the current folder 
    np.savez(f"ablation_random_forest_{meth}_dice_ml_"+date,
            all_percs=all_percs,
            all_times=all_times,
            labs=labs,
            boxes=boxes,
